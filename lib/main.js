...
export default {
	config: {
		...
		forceIndentationType: {
			...
			description: 'Always assume indentation should be one type, rather than scanning the file for the most common indentation.',
		},
	},
	...
	scanFile( textEditor ) {
		const forceIndentationType = atom.config.get( 'linter-mixed-indent.forceIndentationType' );
		const options = {
			comments: atom.config.get( 'linter-mixed-indent.ignoreComments' ),
			indent: forceIndentationType === 'automatic' ? null : forceIndentationType,
		}
